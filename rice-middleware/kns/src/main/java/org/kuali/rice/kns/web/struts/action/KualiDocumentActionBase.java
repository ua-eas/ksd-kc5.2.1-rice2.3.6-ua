...
/**
 * This class handles all of the document handling related actions in terms of passing them from here at a central point
 * to the distributed transactions that actually implement document handling.
 */
public class KualiDocumentActionBase extends KualiAction {
	 ...
	 * <p/>
	 * NOTE: No need to hook into execute for handling framwork setup anymore. Just implement the methodToCall for the
	 * framework setup, Constants.METHOD_REQUEST_PARAMETER will contain the full parameter, which can be sub stringed
	 * for getting framework parameters.
	 *
	...
	public ActionForward execute( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response ) throws Exception {
		...
		if ( form instanceof KualiDocumentFormBase
			...
			if ( isFormRepresentingLockObject( formBase ) ) {
				// form represents a document using the BO class PessimisticLock so we need to skip the authorizations
				// in the next logic check
				...
				// KRADServiceLocatorInternal.getDocumentAuthorizationService().getDocumentAuthorizer(document);
	 ...
	 * various openings and status Additionally it may be nice to have a single dispatcher that can know how to dispatch
	 * to a redirect url for document specific handling but we may not need that as all we should need is the document
	 * to be able to load itself based on document id and then which actionforward or redirect is pertinent for the
	 * document type.
	 *
	 * @param mapping
	 * @param form
	...
	/**
	 * This method loads the document by its provided document header id. This has been abstracted out so that it can be
	...
	protected void loadDocument( KualiDocumentFormBase kualiDocumentFormBase ) throws WorkflowException {
		...
		// re-retrieve the document using the current user's session - remove the system user from the WorkflowDcument
		// object
	...
	/**
	 * This method creates a new document of the type specified by the docTypeName property of the given form. This has
	 * been abstracted out so that it can be overridden in children if the need arises.
	 *
	 * @param kualiDocumentFormBase
	...
	/**
	 * This method will insert the new ad hoc person from the from into the list of ad hoc person recipients, put a new
	 * new record in place and return like normal.
	 *
	 * @param mapping
	 * @param form
	 * @param request
	...
	/**
	 * This method will insert the new ad hoc workgroup into the list of ad hoc workgroup recipients put a nuew record
	...
	/**
	 * This method will delete one of the ad hoc workgroups from the list of ad hoc workgroups to route to based on the
	 * line number of the delete button that was clicked. then it will return
	 *
	 * @param mapping
	 * @param form
	 * @param request
	...
	public ActionForward reload( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response ) throws Exception {
		...
		// if (document instanceof SessionDocumentService &&
		// userSession.retrieveObject(kualiDocumentFormBase.getFormKey()) != null) {
	...
	/**
	 * This method will save the document, which will then be available via the action list for the person who saved the
	 * document.
	 *
	 * @param mapping
	 * @param form
	...
	public ActionForward performRouteReport( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response ) throws Exception {
		...
		// set up the variables for the form if java script is working (includes a close button variable and no back
		// url)
	...
	/**
	 * Calls the document service to disapprove the document, using the disapprove confirmation text
	 *
	 * @param mapping
	 * @param form
	 * @param request
	...
	/**
	 * Convenience method to combine the two lists of ad hoc recipients into one which should be done before calling any
	 * of the document service methods that expect a list of ad hoc recipients
	 *
	 * @param kualiDocumentFormBase
	 * @return List
	 */
	 ...
	 *
	 * @see KualiAction#refresh(org.apache.struts.action.ActionMapping, org.apache.struts.action.ActionForm,
	...
	protected void refreshAdHocRoutingWorkgroupLookups( HttpServletRequest request, KualiDocumentFormBase kualiForm ) throws WorkflowException {
		for ( Enumeration<String> i = request.getParameterNames() ; i.hasMoreElements() ; ) {
			...
			/*
			 * if (parameterName.startsWith("newAdHocRouteWorkgroup[") &&
			 * !"".equals(request.getParameter(parameterName))) { if (parameterName.endsWith(".recipientName")) { int
			 * lineNumber = Integer.parseInt(StringUtils.substringBetween(parameterName, "[", "]")); //check for
			 * namespace String namespaceParam = "newAdHocRouteWorkgroup[" + lineNumber + "].recipientNamespaceCode";
			 * String namespace = KimApiConstants.KIM_GROUP_DEFAULT_NAMESPACE_CODE; if
			 * (request.getParameter(namespaceParam) != null && !"".equals(request.getParameter(namespaceParam).trim()))
			 * { namespace = request.getParameter(namespaceParam).trim(); } KimGroup group =
			 * getIdentityManagementService().getGroupByNamespaceCodeAndName(namespace,
			 ...
			 * kualiForm.getAdHocRouteWorkgroup(lineNumber).setRecipientNamespaceCode(group.getNamespaceCode()); } else
	...
	public ActionForward downloadBOAttachment( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response ) throws Exception {
		...
		if ( attachmentIndex >= 0 ) {
			...
			// make sure attachment is setup with backwards reference to note (rather then doing this we could also just
			// call the attachment service (with a new method that took in the note)
			attachment.setNote( note );

			// since we're downloading a file, all of the editable properties from the previous request will continue to
			// be editable.
	...
	public ActionForward insertBONote( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response ) throws Exception {
		...
		if ( rulePassed ) {
			...
			// certain documents where
			// "saved" doesn't really persist, if you notice any problems with missing notes check this line
			// maintenance document BO note should only be saved into table when document is in the PROCESSED workflow
			// status
			...
			if ( attachment != null ) {
				...
				// save again for attachment, note this is because sometimes the attachment is added first to the above
				// then ojb tries to save
	...
	public ActionForward deleteBONote( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response ) throws Exception {
		...
		if ( attachment != null ) { // only do this if the note has been persisted
			...
			if ( note.getNoteIdentifier() != null ) { // KULRICE-2343 don't blow away note reference if the note wasn't
														// persisted
	 ...
	 * parameter KR-NS/Document/SEND_NOTE_WORKFLOW_NOTIFICATION_ACTIONS to determine which action to take
	 *
	 * @param request
	 * @param note
	...
	/**
	 * Makes calls to the PromptBeforeValidation specified for the document. If the class returns an actionforward, that
	 * forward will be returned (thus controlling how execution occurs), or null.
	 *
	 * @param mapping
	 * @param form
	 * @param request
	...
	/**
	 * @see KualiAction#hideAllTabs(org.apache.struts.action.ActionMapping, org.apache.struts.action.ActionForm,
	 *      javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse)
	 */
	@Override
	...
	/**
	 * @see KualiAction#showAllTabs(org.apache.struts.action.ActionMapping, org.apache.struts.action.ActionForm,
	 *      javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse)
	 */
	@Override
	...
	/**
	 * @see KualiAction#toggleTab(org.apache.struts.action.ActionMapping, org.apache.struts.action.ActionForm,
	 *      javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse)
	 */
	@Override
	...
	private class ReasonPrompt {
		...
		/**
		 * @param questionId
		 *            the question id/instance,
		 * @param questionTextKey
		 *            application resources key for question text
		 * @param questionType
		 *            the {@link org.kuali.rice.kns.question.Question} question type
		 * @param questionCallerMapping
		 *            mapping of original action
		 * @param abortButton
		 *            button value considered to abort the prompt and return (optional, may be null)
		 * @param noteIntroKey
		 *            application resources key for quesiton text prefix (optional, may be null)
		 */
		...
		/**
		 * Obtain a validated reason and button value via a Question prompt. Reason is validated against sensitive data
		 * patterns, and max Note text length
		 * 
		 * @param mapping
		 *            Struts mapping
		 * @param form
