...
//        );
		...
		for ( String roleId : roleIds ) {
			Criteria crit = new Criteria();
			crit.addEqualTo( KIMPropertyConstants.RoleMember.ROLE_ID, roleId );
			crit.addEqualTo( KIMPropertyConstants.RoleMember.MEMBER_TYPE_CODE, memberTypeCode );
			Query query = QueryFactory.newQuery( RoleMemberBo.class, crit );
			Collection<RoleMemberBo> collection = getPersistenceBrokerTemplate().getCollectionByQuery( query );
			for ( RoleMemberBo rm : collection ) {
				boolean add = rm.isActive( new Timestamp( System.currentTimeMillis() ) );
				//if active, then check qualifications.
				if (add) {
					for ( RoleMemberAttributeDataBo attribute : rm.getAttributeDetails() ) {
						if ( qualification.containsKey( attribute.getKimAttributeId() ) ) {
							add &= ( qualification.get( attribute.getKimAttributeId() ).equals( attribute.getAttributeValue() ) );
						}
					}
				}
				if ( add ) {
					roleMemberBos.add( rm );
				}
			}
		}
        return roleMemberBos;
