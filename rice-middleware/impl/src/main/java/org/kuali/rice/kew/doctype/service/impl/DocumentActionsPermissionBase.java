...
/**
 * Base class which implements default KIM permission checks for various workflow document actions. This implementation
 * can be used as a base class for a DocumentTypePermissionService or DocumentTypeAuthorizer implementation
 * 
...
public class DocumentActionsPermissionBase {
	...
	/**
	 * Implements
	 * {@link org.kuali.rice.kew.doctype.service.DocumentTypePermissionService#canInitiate(String, org.kuali.rice.kew.doctype.bo.DocumentType)}
	 */
	...
	/**
	 * Implements
	 * {@link org.kuali.rice.kew.doctype.service.DocumentTypePermissionService#canRoute(String, org.kuali.rice.kew.routeheader.DocumentRouteHeaderValue)}
	 */
	...
	/**
	 * Provides base implementaiton for
	 * {@link org.kuali.rice.kew.doctype.service.DocumentTypePermissionService#canSuperUserApproveSingleActionRequest(String, org.kuali.rice.kew.doctype.bo.DocumentType, java.util.List, String)}
	 */
	...
	/**
	 * Implements
	 * {@link org.kuali.rice.kew.doctype.service.DocumentTypePermissionService#canSuperUserApproveDocument(String, org.kuali.rice.kew.doctype.bo.DocumentType, java.util.List, String)}
	 */
	...
	/**
	 * Implements
	 * {@link org.kuali.rice.kew.doctype.service.DocumentTypePermissionService#canSuperUserDisapproveDocument(String, org.kuali.rice.kew.doctype.bo.DocumentType, java.util.List, String)}
	 */
	...
	/**
	 * Implements
	 * {@link org.kuali.rice.kew.doctype.service.DocumentTypePermissionService#canCancel(String, org.kuali.rice.kew.routeheader.DocumentRouteHeaderValue)}
	 */
	...
	/**
	 * Implements
	 * {@link org.kuali.rice.kew.doctype.service.DocumentTypePermissionService#canRecall(String, org.kuali.rice.kew.routeheader.DocumentRouteHeaderValue)}
	 */
	...
	/**
	 * Implements
	 * {@link org.kuali.rice.kew.doctype.service.DocumentTypePermissionService#canBlanketApprove(String, org.kuali.rice.kew.routeheader.DocumentRouteHeaderValue)}
	 */
	...
	/**
	 * Implements
	 * {@link org.kuali.rice.kew.doctype.service.DocumentTypePermissionService#canSave(String, org.kuali.rice.kew.routeheader.DocumentRouteHeaderValue)}
	 */
	...
	/**
	 * Generates permission details map for KIM permission checks. Details are derived from document type, status,
	 * action request code (if non-null) and routeNode name (if non-null). If the document status is not a routed state,
	 * "PreRoute" is used. Note that this has to match the required data defined in krim_typ_attr_t for the krim_typ_t
	 * with srvc_nm='documentTypeAndNodeOrStatePermissionTypeService'. TODO: See KULRICE-3490, make assembly of
	 * permission details dynamic based on db config
	 * 
	 * @param documentType
	 *            the KEW DocumentType
	 * @param documentStatus
	 *            the document status
	 * @param actionRequestCode
	 *            action request code if applicable
	 * @param routeNodeName
	 *            routeNode name if applicable
	...
	/**
	 * This method generates the permission details for the given document with current active route nodes. This method
	 * simply invokes
	 * {@link #buildDocumentTypePermissionDetails(org.kuali.rice.kew.doctype.bo.DocumentType, String, String, String)}
	 * for each node (or once if no node names are provided).
	 * 
	 * @param documentType
	 *            the DocumentType
	 * @param routeNodeNames
	 *            active route nodes for which to generate permission details
	 * @param documentStatus
	 *            document status
	 * @param actionRequestCode
	 *            action request code if applicable
	...
	/**
	 * Generates role qualifiers for authorization check. If the document status is a non-routed status, "PreRoute" is
	 * used. The namespaceCode attribute is derived from the KRAD DataDictionary if there is a mapping for the document
	 * type.
	 * 
	 * @param document
	 *            the document instance
	 * @param routeNodeName
	 *            name of the applicable routenode
	...
	/**
	 * Returns whether to invoke KIM to perform permission checks. First consults the
	 * {@link KewApiConstants#KIM_PRIORITY_ON_DOC_TYP_PERMS_IND} system parameter to determine whether we should check
	 * for permission existence. If this parameter is unset or is true, we proceed to invoke
	 * {@link PermissionService#isPermissionDefinedByTemplate(String, String, java.util.Map)} to determine whether the
	 * given permission is defined anywhere in the system.
	 * 
	 * @param namespace
	 *            namespace of permission we are querying
	 * @param permissionTemplateName
	 *            template name of permissions we are querying
	 * @param permissionDetails
	 *            details of permissions we are querying
	 * @param checkKimPriorityInd
	 *            whether to consult the {@link KewApiConstants#KIM_PRIORITY_ON_DOC_TYP_PERMS_IND} parameter to
	 *            determine whether the check for permission definition
	 * @return whether there are any permissions defined for the given permission template, or false if we are checking
	 *         the kim priority indicator and the {@link KewApiConstants@KIM_PRIORITY_ON_DOC_TYP_PERMS_IND} system
